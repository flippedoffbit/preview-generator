...
{
    if (c == EOF || stream->isClosed || !stream->isReader || stream->rdNext == stream->stopUnget)
        return EOF;
    --(stream->rdNext);
...
static void WriteCloseRecords(struct FCGX_Stream *stream)
{
     ...
     */
    if (!(data->type == FCGI_STDERR && stream->wrNext == data->buff && !data->isAnythingWritten))
    {
        FCGI_Header header;
...
static void EmptyBuffProc(struct FCGX_Stream *stream, int doClose)
{
    ...
    if (!data->rawWrite)
    {
        ...
        if (cLen > 0)
        {
            ...
            *((FCGI_Header *)data->buff) = MakeHeader(data->type,
...
static int ProcessManagementRecord(int type, FCGX_Stream *stream)
{
    ...
    if (type == FCGI_GET_VALUES)
    {
        ...
        *((FCGI_Header *)response) = MakeHeader(FCGI_GET_VALUES_RESULT, FCGI_NULL_REQUEST_ID,
                                                len, paddedLen - len);
        FreeParams(&paramsPtr);
    }
    else
    {
        ...
        ((FCGI_UnknownTypeRecord *)response)->header = MakeHeader(FCGI_UNKNOWN_TYPE, FCGI_NULL_REQUEST_ID,
                                                                  len, 0);
...
{
    FCGX_Stream_Data *data = (FCGX_Stream_Data *)stream->data;
    if (data->reqDataPtr->role != FCGI_FILTER || !stream->isReader || !stream->isClosed || data->type != FCGI_STDIN)
    {
